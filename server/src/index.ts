...
import aiGuideRoutes from './routes/aiGuides.js';
...
/** Render / reverse proxy + Vercel API proxy forward X-Forwarded-For. Without this, rate limits see one IP and block signups. */
if (isProduction) {
...
/** Auth first so signup/login are not starved by the general /api bucket during onboarding. */
...
app.use('/api/ai-guides', aiGuideRoutes);
