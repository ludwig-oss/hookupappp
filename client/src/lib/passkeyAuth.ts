...
export async function registerPasskey(): Promise<string> {
  const optsRes = await axios.post(`${API_URL}/passkey/register/options`);
  const attResp = await startRegistration({ optionsJSON: optsRes.data });
  const verifyRes = await axios.post(`${API_URL}/passkey/register/verify`, attResp);
  return verifyRes.data.message || 'Passkey registered';
}
